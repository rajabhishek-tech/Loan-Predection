...
def user_input_features():
    ...
    Married_No = st.sidebar.radio("Marital Status: ", ('Married', 'Unmarried'))
    
    if Married_No == 'Unmarried':
        Married_No = 1
    else:
        Married_No = 0
    ...
    topdata = {'Married_No':Married_No,'Dependents':Dependents,'Credit_History':Credit_History,'Property_Area':Property_Area,'LoanAmount':LoanAmount,'Loan_Amount_Term':Loan_Amount_Term,'ApplicantIncome':ApplicantIncome,'CoapplicantIncome':CoapplicantIncome}
...
topfeatures = {'Married_No':Married_No,'Dependents':Dependents, 'Credit_History':Credit_History, 'Property_Area':Property_Area, 'LoanAmount_t':LoanAmount_t
...
loandata = pd.read_csv("https://raw.githubusercontent.com/amit0902/Loan-Prediction-Deployment/main/traintop15.csv")

X = loandata.iloc[:,0:15]
y = loandata.iloc[:,15]
